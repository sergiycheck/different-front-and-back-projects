...
const getAuthorById = async (authorId) => {
  const authorDoc = await AuthorModel.findById(authorId).populate({ path: "posts", select: "_id text owner" }).exec();
  return {
    author: {
      id: authorDoc._id,
      name: authorDoc.name,
...
const updateAuthor = async ({ authorId, name }) => {
  const updateResult = await AuthorModel.findOneAndUpdate({ _id: authorId }, { name }, { runValidators: true });
  return updateResult;
};
